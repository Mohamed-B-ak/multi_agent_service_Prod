...
def get_understand_and_execute_task():
    ...
    return Task(
        description=(
            ...
            "  • مثال: \"هل يوجد عميل باسم محمد؟\" → \"نعم، يوجد عميل باسم محمد.\" أو بالإنجليزية: \"Yes, there is a customer named محمد.\"\n"
            "- لا تُدرج معرفات أو تفاصيل إضافية (ID، بريد، هاتف، حالة) ما لم يطلبها المستخدم صراحةً.\n\n"
            ...
            "5. في *الإرسال*: صِغ المحتوى ثم مرّره للوكيل المناسب بعد التحقق من بيانات التواصل (إن لزم اسأل المستخدم مباشرة وباختصار).\n"
            "6. في *الكود*: ولِّد كود بايثون نظيفًا.\n"
            "7. في *قاعدة البيانات*: نفّذ عمليات MongoDB بعد التحقق من الأوامر.\n"
            "8. في الأسئلة المباشرة (نعم/لا/حقائق): أجب بإيجاز شديد دون تفاصيل إضافية.\n"
            "9. قدّم تأكيدًا واضحًا فقط عند أفعال التنفيذ (مثل الإرسال) وبأقصر صياغة ممكنة.\n"
            "10. تعامل مع الأخطاء بلطف وبجمل قليلة ومباشرة.\n\n"

            "إجراءات السلامة:\n"
            "- لا ترسل بريد/واتساب/مكالمات إلا بطلب صريح بكلمة 'إرسال' أو ما يعادلها.\n"
        ...
        expected_output=(
            ...
            "✅ في أسئلة نعم/لا: إجابة قصيرة فقط (مثال: \"نعم، يوجد عميل باسم محمد.\").\n"
            "✅ في الصياغة: النص المطلوب فقط دون شروح إضافية.\n"
            "✅ في الإرسال: تأكيد الإرسال بجملة قصيرة + المحتوى إذا لزم.\n"
            "✅ في الكود: كود بايثون فقط مع أقل قدر من الشرح (أو دون شرح إن لم يُطلب).\n"
            "✅ في قاعدة البيانات: النتيجة المطلوبة فقط (قيمة/سجل/نجاح العملية) دون تفصيل غير مطلوب.\n"
