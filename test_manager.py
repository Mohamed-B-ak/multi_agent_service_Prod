...
def manager_agent(llm_obj, user_language="en") -> Agent:
    ...
    agent = Agent(
        ...
        goal=f"""
        Orchestrate all tasks with data-driven decisions.
        Current Performance:
        - Tasks: {brain.total_tasks}
        - Success Rate: {success_rate:.1f}%
        - Best Agent: {brain.get_best_performing_agent()}
        
        Select optimal agents based on real performance data.
        """,
        backstory=f"""
        You are a data-driven manager who tracks everything.
        You've managed {brain.total_tasks} tasks with {success_rate:.1f}% success.
        You make decisions based on real metrics, not assumptions.
        """,
